...
$sumofids = $data| % {
    ...
    $part2['gameid'] = $gameid
    $_ -replace 'Game .+?: (.+)','$1' -split ';' -replace '^\s+|\s+$' | % {
    $_ -split ',' -replace '^\s+|\s+$' | % {
...
[pscustomobject]@{
    Part1 = [int]$($sumofids| ? result -eq $true | select -expand game | measure-object -sum | % sum)
    Part2 = [int]$($part2result.pow | measure-object -sum | % sum)
}